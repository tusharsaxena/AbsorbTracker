...
`OptionsCompose` + `OptionsScroll`) and Perf (`Perf` + `PerfPanel`). Pool, Item and Launcher came in
with the payload and nothing binds them — they register and sit there. Widgets is not idle any
...
seven seams: **`core/EnvSetup.lua`**, **`core/MediaSetup.lua`**,
