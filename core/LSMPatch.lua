...
-- currentVersion + 1 to win the version race, and per-instance hide the displayButton plus
-- re-anchor frame.label and frame.DLeft to the frame's left edge so the empty 42px slot
-- collapses. The popup's per-row hover preview is unaffected.
--
-- LSM30_Font / LSM30_Statusbar use AGSMW:GetBaseFrame (no displayButton), so this is
-- Border-specific.
function NS.ApplyLSMBorderPatch()
    ...
    AceGUI:RegisterWidgetType("LSM30_Border", function()
        ...
        if f and f.displayButton then
            ...
            -- DLeft is the left cap of the CharacterCreate-LabelFrame dropdown bar. Upstream
            -- AGSMW:GetBaseFrameWithWindow repositions it to displayButton.BOTTOMRIGHT; restore
            -- the original GetBaseFrame anchor so the bar starts at the frame's left edge again.
            if f.DLeft then
