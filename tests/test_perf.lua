...
test("perf: cancel is offered throughout a run and nowhere else", function()
...
test("perf: cancel stays clickable while a run is mid-experiment", function()
  reset()
  P.Start("c")
  P.Measure("a")
  tick(0.5, true)
  NS.PerfPanel:Show()
  assertFalse(NS.PerfPanel.IsActionable("measureA"), "A is busy, not clickable")
  assertFalse(NS.PerfPanel.IsActionable("finish"), "finish still locked")
  assertTrue(NS.PerfPanel.IsActionable("cancel"), "but the way out is always there")
  P.Cancel()
  settle()
  NS.PerfPanel:Hide()
end)

test("perf: cancel is not clickable once the run is finished", function()
  reset()
  P.Start("c")
  P.Measure("a"); tick(0.5, true); tick(0.5, false)
  P.Stop()
  NS.PerfPanel:Show()
  assertEqual(NS.PerfPanel.StateOf("cancel"), "locked", "greyed out")
  assertFalse(NS.PerfPanel.IsActionable("cancel"), "and inert")
  NS.PerfPanel:Hide()
end)

-- ── showing and hiding the panel ────────────────────────────────────────────────────────────

test("perf: hiding the panel never touches the run", function()
...
test("perf: Toggle flips visibility both ways", function()
  reset()
  NS.PerfPanel:Hide()
  NS.PerfPanel:Toggle()
  assertTrue(NS.PerfPanel:IsShown(), "shown")
  NS.PerfPanel:Toggle()
  assertFalse(NS.PerfPanel:IsShown(), "and hidden again")
end)

test("perf: the close button hides the panel and leaves the run alone", function()
  reset()
  P.Start("close")
  NS.PerfPanel:Show()
  local f = NS.PerfPanel.__frame()
  assertTrue(f.closeButton ~= nil, "there is a close button")
  f.closeButton:__fire("OnClick")
  assertFalse(NS.PerfPanel:IsShown(), "it hid the panel")
  assertTrue(P.run, "and the run carried on")
  P.Cancel()
  settle()
end)
...
test("perf: report and dump stay clickable after use, but read as done", function()
  ...
  assertTrue(NS.PerfPanel.IsActionable("report"), "and is still clickable")
  assertEqual(P.Progress().dump, "ready", "dump is independent")
end)

test("perf: marking a review action twice is a no-op", function()
  reset()
  P.Start("review")
  P.MarkReviewed("dump")
...
test("perf: MarkReviewed ignores keys that are not review actions", function()
  reset()
  assertFalse(P.MarkReviewed("finish"), "finish is not a review action")
  assertFalse(P.MarkReviewed("nonsense"), "nor is anything else")
end)

test("perf: a fresh run clears the review marks", function()
  reset()
  P.Start("first")
...
test("perf: the panel titles itself like the debug console", function()
  reset()
  NS.PerfPanel:Show()
  assertTrue(NS.PerfPanel.__frame().title ~= nil, "it has a title")
  NS.PerfPanel:Hide()
end)

test("perf: every step label names what it acts on", function()
