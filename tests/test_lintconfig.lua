-- tests/test_lintconfig.lua — the "no blanket suppression" gate (lint, `M4-11`).
--
...
-- `ignore = { "212/self", "212/event", "211/addonName", "431" }` through the whole 2026-09-07
-- remediation. Three of those four entries name the variable, which is what makes this repo's
-- version of the fault the instructive one: the entries LOOKED narrow and were not, because a
-- top-level `ignore` reaches every file whatever it names. Removing the four lines reported
-- thirty-two findings, and nineteen of them were not conventions at all — nineteen files opened
-- `local addonName, NS = ...` over a folder name they never read, and `M4c-06` fixed all nineteen
-- at source rather than re-parking them in a narrower suppression. Two of the four entries were
-- silencing nothing whatsoever: this tree produces no `212/event` and no `431`, and the `431`
-- comment described shadowing a GLOBAL, which is not what `431` is.
--
-- Rule 3 is the one this repo needs most, and rule 1 alone would not have caught it. The thirteen
-- `212/self` that survived are real receivers, and they now sit in three stanzas keyed to one file
-- each. The measurement that says the difference is real: an unused `self` on a throwaway method in
-- modules/Bar.lua, a fresh `local addonName, NS = ...` header, and a genuine shadowed upvalue each
-- report under the config this gate guards and reported nothing under the blanket.
--
-- Removing the four lines is one afternoon's work. Keeping them removed is what this file is for.
--
...
-- existing. Same bargain tests/_kit/test_eol.lua strikes.
...
-- would cover every 212 the blanket did — which is why the list is here and not just `ignore`.
...
test("lintconfig: .luacheckrc sets no top-level ignore", function()
  ...
  if ignore ~= nil then
    ...
    fail(".luacheckrc sets a top-level `ignore` of { " .. table.concat(shown, ", ") .. " }. A "
      .. "top-level ignore reaches all 54 files including the ones with no business producing the "
      .. "code, so it reads as coverage and provides none (lint, `M4-11`) — and naming the "
      .. "variable does not rescue it, which is exactly how the four entries `M4c-06` removed "
      .. "survived so long. Move each code into a `files[...]` stanza naming the file that earns "
      .. "it, or put a `-- luacheck: ignore <code>` beside the single line that needs it", 2)
  end
end)
...
test("lintconfig: .luacheckrc switches no warning class off wholesale", function()
  ...
  if #off > 0 then
    fail(".luacheckrc turns a whole warning class off at the top level: "
      .. table.concat(off, ", ") .. ". That is a blanket ignore spelled as a switch, and lint "
...
test("lintconfig: no source file carries a bare inline luacheck ignore", function()
  ...
  for _, path in ipairs(trackedLua()) do
    ...
    for line in fh:lines() do
      ...
      -- this rule steers towards; tests/test_schema.lua's exempt-key branch is the repository's
      -- one user of it, and carried the bare form until `M4c-06`.
  ...
  if #bare > 0 then
    fail("bare `-- luacheck: ignore` directives, which silence every code in scope: "
      ...
      .. "warning in the same scope is still reported (lint)", 2)
  end
end)
