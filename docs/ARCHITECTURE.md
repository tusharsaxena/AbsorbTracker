...
| `/at perf <sub>` | The performance probe (`core/Perf.lua`): `start [label]`/`finish` bracket a run, `measure a\|b` arm a combat-gated experiment, `report` print it, `dump` emit JSON, `cancel` abandon it unsaved. `measure b` owns the suspend; there is no manual verb for it. Its own `NS.COMMANDS` verb. See [performance.md](./performance.md) |
