...
- **(Ka0s standard architecture-§5)** the `path` resolves against `NS.defaults.profile` via `NS.ResolvePath` — or, for a `global.` path, against `NS.defaults.global` — dotted per-unit paths (`units.target.barWidth`) walk the nested `units` table the same way flat paths (`locked`) index directly, so a typo anywhere in a per-unit path is caught the same as a flat one. Rows on the `profiles` page are exempt (their options are AceDBOptions-supplied). So is a row that owns its storage through its own `get` (architecture-§5's closure-backed exemption): the minimap row, whose path `global.minimap.shown` names the row's sense while its value is LibDBIcon's `db.global.minimap` `hide` key, inverted by the row's get/set, so there is no `shown` default to resolve.
...
## With LibKa0s absent: hollow composers and the write-through list

On a library-less load `settings/Schema.lua` falls back to its write-completing, log-silent stub, and `settings/OptionsSetup.lua`'s stub answers each of the five composers (`MasterControls`, `BarGroup`, `BorderGroup`, `FontGroup`, `ColorPair`) with an **empty block** (options-ui-§1; a host copy of a composer is anti-pattern #73). So the degraded schema is the **hand-written rows alone** — 15 of the 70 — and the 55 composed rows are the one named gap: Master controls 7, and per unit Bar 4, Background 2, Border 4, Text 6. `tests/test_perf.lua` pins all three figures and the per-tab attribution.

Two composed paths are still written by host code on that load: `enabled` (`/at enable`, `/at disable`) and `locked` (`/at lock`, `/at unlock`, the combat re-lock, the launcher menu's Locked entry — which runs `/at lock` / `/at unlock` itself). They are declared in `settings/Schema.lua`'s `WRITE_THROUGH = { "enabled", "locked" }`, handed to `SchemaLib:New` as `writeThrough` (LibKa0s-Schema-1.0 minor 2, options-ui-§1 route (a)) on both the live instance and the stub. A write to a listed path with no row is stored raw and announced with a synthetic `{ path, writeThrough = true }` row. The `announce` sees `row.writeThrough` and dispatches the host's own reaction from `NS.MasterReactions` (the `masterOnChange` table `settings/General.lua` publishes), so the enable edge still moves the stand-down latch and an in-combat unlock is still refused. A path with a row always takes the row, so on a full load the list changes nothing. Any other row-less path is still refused. Reads need no row: `NS.GetSetting` walks the store and falls back to the shipped defaults.
...
- **The non-key/value verbs.** `help`, `config`, `enable`, `disable`, `lock`, `unlock`, `toggle`, `debug`, `diagnostics`, `perf`, `update`, `version`, `resetposition`, and `profile` live as dedicated entries in the ordered `NS.COMMANDS` table in `settings/Slash.lua` — 19 verbs in all, counting the five schema-driven ones (`list`, `get`, `set`, `reset`, `resetall`). `enable` and `disable` are the reserved pair and are **aliases** for the `enabled` row (slash-commands-§2): they write that path through `NS.SetByPath` and hold no state of their own. The table is passed **into** LibKa0s-Slash-1.0 rather than owned by it: the About page renders the same rows (via `NS.Slash:LandingRows()`, the library's formatted list), and a library that owned the table would drag the options surface into depending on it. An unknown verb prints `unknown command '<verb>'` then the help list.
...
|------|------|---------|----------------|-------------|
| `enabled` | bool | `true` | — | **New.** The addon-wide switch (options-ui-§15) — "turn this off without unloading it". **Not a draw gate** (slash-commands-§7): its `onChange` is `NS.SyncEnabledHold()`, which takes or releases the `disabled` hold on the lifecycle latch (`core/Lifecycle.lua`), and standing down unregisters every event, bus subscription and timer in the same turn. `NS.ShouldShowBar` reads the latch, never this value. It sits above the three per-unit flags and does **not** replace them: those are *which bars exist*, this is *whether the addon runs at all*, and §15 forbids conflating an addon-wide row with a per-instance one. Label "Enable Absorb Tracker". **Master controls** tab, order 0. |
