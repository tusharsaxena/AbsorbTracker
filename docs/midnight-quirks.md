...
This asymmetry is the whole gotcha, and it shapes the debug-logging design (Ka0s §8/§9). The single debug sink, `NS.Debug(tag, fmt, ...)` (`core/DebugLog.lua`), routes every vararg through `NS.SafeToString` then `fmt:format(...)`, so no call site can be killed by a secret — but the *hot path* is also **coalesced** (§9): `OnAbsorbChanged` (`core/AbsorbTracker.lua`) and `UpdateAbsorbBar` (`modules/Display.lua`) no longer log a line per absorb event / per repaint. They bump debug-gated counters (`dbgAbsorbEvents`, and `NS.NoteRepaint()`'s `dbgRepaints`), and `OnLeaveCombat` flushes one `[Combat] left: N events, M repaints` rollup at combat end.
...
The guard lives at the concat boundary, not the call sites: `NS.SafeToString` (`core/Util.lua`) substitutes the sentinel `"<secret>"` for any value that a real `table.concat` would reject. Detection **must probe `table.concat`, not `..`** — `NS.IsConcatSafe(v)` runs `pcall(function() return table.concat({ v }) end)`, because a probe built on `..` reports a secret as *safe* (the operator propagates instead of raising) and lets it slip straight through to the real concat. Both `NS.Print` (chat) and `NS.Debug` (console) route every arg through `SafeToString`, so no addon line can be killed by a secret even outside the coalesced hot path. `nil`/`boolean` are handled up front (they are never secret but `table.concat` rejects them too). All debug work remains gated behind `NS.State.debug` so the probe cost is zero when debug is off.
...
If Blizzard removes an event the addon listens for (it has happened in past retail patches — `LEARNED_SPELL_IN_TAB` was replaced by `LEARNED_SPELL_IN_SKILL_LINE`), the failure mode is that the registration succeeds but the event never fires. AbsorbTracker listens for `UNIT_ABSORB_AMOUNT_CHANGED` and `UNIT_MAXHEALTH` (on a private `RegisterUnitEvent(event, "player")` frame — a documented §9.1 deviation, see [ARCHITECTURE.md → Standards Deviations](./ARCHITECTURE.md#standards-deviations)) and `PLAYER_ENTERING_WORLD` / `PLAYER_REGEN_DISABLED` / `PLAYER_REGEN_ENABLED` (via `self:RegisterEvent(...)`), all in `addon:OnEnable` (`core/AbsorbTracker.lua`), plus the AceAddon lifecycle events `ADDON_LOADED` → `OnInitialize` and `PLAYER_LOGIN` → `OnEnable` — all stable retail events. If a future patch deprecates one, the replacement should be wired by patching the registration in `core/AbsorbTracker.lua`; nothing else needs to change.
