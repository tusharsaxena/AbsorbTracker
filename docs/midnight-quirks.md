...
The event can fire many times per second during raid encounters with stacking absorbs (Power Word: Shield + trinket procs + Discipline absorbs + …). The `OnAbsorbChanged` handler in `core/AbsorbTracker.lua` is intentionally debug-only — it records a debug line (gated behind `NS.State.debug`) and does nothing else. Using it to drive `UpdateAbsorbBar` directly would over-render the StatusBar and the string-format calls.

The periodic repaint ticker is the source of truth for visual updates. It runs on **AceTimer** — `NS.RestartUpdateTicker` (`modules/Timer.lua`) schedules `NS.UpdateAbsorbBar` via `NS.addon:ScheduleRepeatingTimer(...)` and tears it down with `NS.addon:CancelTimer(...)` (Ka0s standard §3.1 — no raw `C_Timer` for repeating work). `updateInterval` is user-configurable (0.1 – 10 s; default 1.0 s), and an interval-change guard skips a needless timer churn when `/at set updateInterval` is called with the same value. See [data-flow.md](./data-flow.md#absorb-update-path).
