...
This asymmetry is the whole gotcha, and it shapes the debug-logging design (Ka0s §8/§9). The single debug sink, `NS.Debug(tag, fmt, ...)` (`LibKa0s-DebugLog-1.0`, `libs/LibKa0s/DebugLog.lua`, vendored — `core/DebugLogSetup.lua` hands it `NS.SafeToString` as the descriptor's `safeToString` hook and publishes the result under the name the call sites already use), routes every vararg through `NS.SafeToString` then `fmt:format(...)`, so no call site can be killed by a secret — but the *hot path* is also **coalesced** (§9): `OnAbsorbChanged` (`core/AbsorbTracker.lua`) and `UpdateAbsorbBar` (`modules/Display.lua`) no longer log a line per absorb event / per repaint. They bump debug-gated counters (`dbgAbsorbEvents`, and `NS.NoteRepaint()`'s `dbgRepaints`), and `OnLeaveCombat` flushes one `[Combat] left: N events, M repaints` rollup at combat end.
...
If Blizzard removes an event the addon listens for (it has happened in past retail patches — `LEARNED_SPELL_IN_TAB` was replaced by `LEARNED_SPELL_IN_SKILL_LINE`), the failure mode is that the registration succeeds but the event never fires. AbsorbTracker listens for `UNIT_ABSORB_AMOUNT_CHANGED` and `UNIT_MAXHEALTH` (on a private `RegisterUnitEvent(event, "player")` frame — a documented events-frames-taint-§1 deviation, see [ARCHITECTURE.md → Standards Deviations](./ARCHITECTURE.md#standards-deviations)) and `PLAYER_ENTERING_WORLD` / `PLAYER_REGEN_DISABLED` / `PLAYER_REGEN_ENABLED` (via `self:RegisterEvent(...)`), all in `addon:OnEnable` (`core/AbsorbTracker.lua`), plus the AceAddon lifecycle events `ADDON_LOADED` → `OnInitialize` and `PLAYER_LOGIN` → `OnEnable` — all stable retail events. If a future patch deprecates one, the replacement should be wired by patching the registration in `core/AbsorbTracker.lua`; nothing else needs to change.
