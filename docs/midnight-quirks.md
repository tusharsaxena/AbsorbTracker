...
AbsorbTracker listens for `UNIT_ABSORB_AMOUNT_CHANGED` and `UNIT_MAXHEALTH` (on a private `RegisterUnitEvent(event, unit)` frame per unit — a documented events-frames-taint-§1 deviation, see [ARCHITECTURE.md → Documented deviations](./ARCHITECTURE.md#documented-deviations)) and `PLAYER_ENTERING_WORLD` / `PLAYER_REGEN_DISABLED` / `PLAYER_REGEN_ENABLED` / `PLAYER_TARGET_CHANGED` / `PLAYER_FOCUS_CHANGED` (AceEvent, through the helper), all from `addon:OnEnable` via `RegisterLifecycleEvents` and `SyncUnitEventFrames` (`core/AbsorbTracker.lua`), plus the AceAddon lifecycle events `ADDON_LOADED` → `OnInitialize` and `PLAYER_LOGIN` → `OnEnable` — all stable retail events. If a future patch deprecates one, the replacement should be wired by patching the registration in `core/AbsorbTracker.lua`; nothing else needs to change.
