...
copied in, not depended on at runtime. The payload now ships **fifteen majors across twenty-seven files**,
...
through `OptionsNav.lua`) and `Perf.lua` (+ `PerfPanel.lua`); `Lifecycle.lua`
and `Launcher.lua` take a descriptor too, and arrived after the five extractions this section is named for.
