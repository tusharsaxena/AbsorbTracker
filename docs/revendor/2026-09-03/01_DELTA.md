...
```
grep -n '[Bb]undles' AbsorbTracker/CLAUDE.md
```
...
```
diff -r --strip-trailing-cr <tag>/LibKa0s AbsorbTracker/libs/LibKa0s   # 1 file: OptionsCompose.lua
diff -r --strip-trailing-cr <tag>/testkit AbsorbTracker/tests/_kit     # empty
```
...
Majors reached from this addon's own source: Core DebugLog Env Media NoSuchModule Options Perf Slash Widgets 
