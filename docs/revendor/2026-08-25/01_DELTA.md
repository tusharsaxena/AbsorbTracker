# 01 — Delta: AbsorbTracker vs LibKa0s v1.15.0
...
```sh
grep -in 'bundles' AbsorbTracker/CLAUDE.md
```
...
```sh
grep -hoE 'local +(MAJOR, )?[A-Z_]*MINOR *= *("[^"]+", *)?[0-9]+' AbsorbTracker/libs/LibKa0s/*.lua
```
...
```sh
diff -r --strip-trailing-cr <scratch>/LibKa0s AbsorbTracker/libs/LibKa0s   # 0 lines
diff -r                     <scratch>/LibKa0s AbsorbTracker/libs/LibKa0s   # 0 lines
diff -r --strip-trailing-cr <scratch>/testkit AbsorbTracker/tests/_kit     # 0 lines
diff -r                     <scratch>/testkit AbsorbTracker/tests/_kit     # 0 lines
```
...
```sh
grep -rnoE 'LibStub\("LibKa0s-[A-Za-z]+-1\.0", true\)' AbsorbTracker --include='*.lua' | grep -v '/libs/' | grep -v '/tests/'
```

- `AbsorbTracker/core/CoreSetup.lua:25` — Core
- `AbsorbTracker/core/DebugLogSetup.lua:14` — DebugLog
- `AbsorbTracker/core/EnvSetup.lua:54` — Env
- `AbsorbTracker/core/MediaSetup.lua:82` — Media
- `AbsorbTracker/core/PerfSetup.lua:14` — Perf
- `AbsorbTracker/settings/OptionsSetup.lua:36` — Options
- `AbsorbTracker/settings/Schema.lua:185` — Slash
- `AbsorbTracker/settings/Slash.lua:24` — Slash
...
```sh
grep -n 'Kit.VERSION' <scratch>/testkit/framework.lua AbsorbTracker/tests/_kit/framework.lua
```
...
| Gate | Command | Result |
|---|---|---|
| Lint | `luacheck .` | 0 warnings / 0 errors in 29 files |
| Tests | `lua tests/run.lua` | 508 passed, 0 failed, 0 skipped |
