...
- `NS.defaults.global.schemaVersion = 0` — the account-wide DB-version stamp. Its default is `0`, the pre-migration floor, and never the current version (savedvariables-§1): AceDB's `removeDefaults` strips a stored value equal to its default at logout, so a current-version default would never persist, and AceDB's defaults merge backfills a declared default onto a legacy account that stored no stamp, so it would read as already migrated. The runner's target is `NS.SCHEMA_VERSION` (**5**), derived in `core/Database.lua` from the ladder's last step.
...
| `/at profile new <name>` | Create `<name>` with default settings AND switch to it (`db:SetProfile(name)` then `db:ResetProfile()`). Refuses a name that already exists — `Profile '<name>' already exists — /at profile use <name> switches to it, /at profile reset resets it` — and changes nothing, so it can never wipe a profile. |
| `/at profile copy <name>` | Copy `<name>`'s settings into the active profile (`db:CopyProfile`). Fires `OnProfileCopied`. Refuses the active profile (`Cannot copy a profile onto itself`) and a name that is not stored (`Profile '<name>' not found — /at profile list shows them`) before AceDB sees it, since AceDB raises a Lua error on both. |
| `/at profile delete <name>` | `db:DeleteProfile(name, true)`. Refuses if `<name>` is the active profile (`Cannot delete the current profile`), and prints the same `not found` line as `copy` for a name that is not stored instead of reporting a deletion that did not happen. |
| `/at profile reset` | Reset the active profile to defaults (`db:ResetProfile()`). Fires `OnProfileReset`. |

The whole subcommand delegates to AceDB's `SetProfile / CopyProfile / DeleteProfile / ResetProfile / GetProfiles / GetCurrentProfile`. `runProfile` first checks `if not db or not db.SetProfile` and prints `Profile system requires AceDB-3.0` when that method is absent (the fallback path), then dispatches through the file-local `PROFILE_VERBS` table — built once at load, keyed by the lowercased sub-verb, each value a `function(db, name)`. **Only the verb is lowercased**; the argument keeps its case, because AceDB profile names are case-sensitive. The four name-taking verbs (`use`, `new`, `copy`, `delete`) are wrapped in `needsName(verb, fn)`, which prints that verb's own `Usage:` line and does nothing when the name is empty — so the guard exists once, not four times. `new`, `copy` and `delete` check the name against `db:GetProfiles()` first (the file-local `profileExists`, exact and case-sensitive), so AceDB never sees a name it would raise on or silently ignore; there is no `pcall`. An unrecognized subcommand prints `Unknown profile subcommand '<sub>'` then re-prints the list. Adding a sub-verb is one `PROFILE_VERBS` entry plus one `PROFILE_HELP` row.
...
3. **Walk the account-wide ladder** — the file-local `SCHEMA_STEPS` array in `core/Database.lua`, an ordered list of `{ to = N, apply = fn }` rows the loop climbs one version at a time: the v2 step (retiring the dead `updateInterval` key from every profile in the store, orphaned when the poll ticker became event-driven), the stamp-only v3 step, the v4 step (dropping the dead `hidden` master toggle from every profile in the store, replaced by the per-unit `enabled` flags), then the v5 step (mapping the `showOnlyInCombat` boolean onto the `visibility` dropdown on every profile, then deleting the boolean), landing on `global.schemaVersion = 5`. Each step logs one `[Migrate]` debug line only when the bump actually happens, and the step's own output lands before its version line. The runner alone moves the stamp, and only past a step that returned: each `apply` runs under `pcall`, and one that raises leaves the stamp at the last completed step, stops the ladder and prints one chat line naming the stop, so the next load retries it. **A new schema version is one new row in `SCHEMA_STEPS`**; `NS.SCHEMA_VERSION` follows it without a second edit.
...
Each bar's saved position lives at `db.profile.units.<unit>.position = { point, relPoint, x, y }`, read and written only through `NS.Units.Position` / `NS.Units.SetPosition` (`core/Units.lua`). It is **not** a schema row — it is written by dragging, architecture-§5 named non-setting state whose owner and writers are named in [schema.md → Named non-setting state](./schema.md#named-non-setting-state) — and it is **never mirrored**: a mirrored position would stack all three bars on one spot, so `position` (like `enabled`) stays per-unit even while a unit mirrors the player's appearance.
...
Clearing positions goes through **one** implementation, `Helpers.ResetAllPositions` (`settings/UnitPanel.lua`): it calls `NS.Units.SetPosition(unit, nil)` for every unit in `NS.Units.LIST` and republishes `MSG.POSITION`. Both entry points delegate to it — `/at resetposition` (`settings/Slash.lua`) and the **Reset Position** button on the General page (`settings/General.lua`). `Helpers.RestoreAllDefaults` used to be a third, through an `afterRestoreAll` hook, because `position` is written by dragging rather than by a schema row and `ApplyDefault` never touches it. It is not any more: options-ui-§12 makes Reset All Settings a **profile reset** (the descriptor's `resetProfile` → `db:ResetProfile()`), and a position lives *in* the profile, so it comes back with everything else. They diverged here once: the panel button used to nil the pre-v3 flat `db.profile.position`, a key the v3 migration deletes, so it cleared an already-nil key and every bar re-anchored from its untouched `units.<unit>.position` — a silent no-op. Do not re-inline the loop at any call site.
