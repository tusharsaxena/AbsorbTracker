...
| 18 | `settings/Panel.lua` | 130 | Settings UI registration shell. Publishes empty `NS.Helpers` for the toolkit slices to decorate, sets `NS.PARENT_TITLE`, owns the `pendingPages` queue. `RegisterOptionsPage(key, name, builder)` appends to it; `CreateOptionsPanel()` (enable-time drain) stashes `NS.AceGUI` once, runs `ValidateSchema`, registers the top-level canvas via `Helpers.CreatePanel`, defers `Helpers.BuildMainContent` to first OnShow, then runs each queued page builder; `OpenOptionsPanel()` is combat-lockdown gated — in combat it **refuses** (prints a grey `[AT]` notice and returns, options-ui-§2), never touching the protected category-switch and never deferring; `RefreshOptionsPanel()` routes to `Helpers.RefreshAllPanels`. See [settings-panel.md](./settings-panel.md). |
