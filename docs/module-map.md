...
NS.Units.CopyFromPlayer(unit)  -- one-shot: deep-copies the player's 19 appearance keys onto
                               -- `unit`, then clears mirror. Twenty NS.SetByPath calls, not twenty
                               -- table writes, so the [Set] log sees them (debug-logging-§10) and
                               -- each row's own onChange fires. Does NOT copy `position` or
