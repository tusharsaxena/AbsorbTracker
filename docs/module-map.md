...
NS.Helpers
    ...
    Helpers.RenderUnitPanel(ctx, pageKey)          -- Appearance: Unit banner + mirror header
...
Each runs at file-load time and calls `NS.RegisterOptionsPage(key, name, build)`. General registers its per-unit `units.<unit>.enabled` rows first (so the **Bars** tab leads the strip), then calls `NS.RegisterSchemaRows({...})` for its three unit-agnostic globals (`showOnlyInCombat`, `locked`, `throttleWindow` — the fourth was `hidden`, dropped by schema v4). Appearance instead defines `addUnitRows(unit)` and calls it once per `NS.Units.LIST` entry, registering its eighteen appearance keys three times (path prefixed `units.<unit>.`, tagged `unit = unit`) — plus a group-less `mirror` row (`skipRender = true`) for target/focus only. Profiles registers no rows; its UI is AceDBOptions-supplied. At enable time, the `build` closure calls `Helpers.RenderTabbedSchema(ctx, pageKey, afterGroup, pairWith)` (General — a two-tab strip, no banner) or `Helpers.RenderUnitPanel(ctx, pageKey)` (Appearance — Unit banner, mirror header, five-tab strip), with **no** `rendered` one-shot guard on the latter (`RenderUnitPanel` does a full rebuild every call, by design — unit switches, tab clicks and mirror toggles need it). The LSM-backed rows on Appearance set `dialogControl = "LSM30_Statusbar" | "LSM30_Border" | "LSM30_Font"` and `values = NS.Helpers.LSMValues(mediaType)`. `Profiles.build` returns nil (opting the page out) when AceDBOptions / AceConfigDialog / AceConfig / AceGUI aren't all present.
...
| 8 | `core/Data.lua` | 202 | Bar data + media access layer. `NS.db` ref, `GetSetting(path)` / `SetSetting(path, value)` over `db.profile` (dotted-path aware via `NS.ResolvePath`/`NS.SetPath`, falling back to `flatDefaults`), the cached LSM fetchers (`GetBarTexture(unit)` / `GetBgTexture(unit)` / `GetBorder(unit)` / `GetFont(unit)` with `FALLBACK_*`, each resolved through `NS.Units.Get`), `GetLSM` / `ClearLSMCache`, and the **four** class-color-aware color getters (`GetBarColor` / `GetBgColor` / `GetBorderColor` / `GetFontColor`, each `(unit)`) that re-read that unit's `useClassColor*` at call time — the class color itself is always the player's. All four go through one file-local `resolveColor(c, on, classColor)`, which is where the three collection-wide rules live so four copies cannot drift: **the configured alpha survives the mode** (a class color carries none of its own), **an unknown class keeps the configured color** (`GetPlayerClassColor` / `GetBgClassColor` answer **nil** rather than the opaque white they used to invent, and the caches fill only on success so a class that resolves a moment later is still picked up), and the swatch is read under **both** modes, which is why no color row carries `disabledIf`. `NS.GetBarAlpha(unit)` is the fifth reader: the whole frame's opacity, **clamped** to the row's own 0.1–1, because the value comes out of SavedVariables and an alpha of 0 is an invisible bar with no error. |
| 9 | `core/Units.lua` | 118 | `NS.Units` — the only file that reads `db.profile.units` for appearance. `LIST`/`LABEL` (player/target/focus identity), `APPEARANCE_KEYS` (the 18 per-unit keys; `tests/test_units.lua` asserts the list and the schema's per-unit rows agree in **both** directions, because a key in one and not the other is silently half-broken), `Config(unit)`, `IsEnabled(unit)`, `IsMirrored(unit)` (always false for player), `SourceUnit(unit)`, `Get(unit, key)` (the mirror-resolved read path), `Set(unit, key, value)` (writes the unit's own config, never mirror-resolved — published without a production caller as the write half of the `Get` seam; `/at set` does **not** come through here), `DeepCopy(v)` (the addon's ONE recursive table copy — `core/Database.lua` uses it too), `Position(unit)` / `SetPosition(unit, pos)` (never mirrored), and `CopyFromPlayer(unit)` (one-shot deep-copy of the 18 appearance keys, then clears `mirror`; does not copy `position` or `enabled`). |
...
| # | File | Lines | Responsibility |
|---|------|-------|----------------|
| 14 | `defaults/Profile.lua` | 103 | AceDB-shaped defaults. Three flat globals (`locked`, `showOnlyInCombat`, `throttleWindow` — no `hidden`; v4 dropped it and the per-unit `enabled` flags are the visibility switch) plus `NS.defaults.profile.units.{player,target,focus}` — each unit's own appearance table, built by an `appearance()` factory so no table is shared across units (target/focus additionally get `enabled = false, mirror = true`; player gets `enabled = true, mirror = nil`, since the player is the mirror source). `NS.defaults.global.schemaVersion = 4` (the account-wide, DB-wide migration stamp) and `NS.defaults.profile.schemaVersion = 1` (the per-profile lift stamp — default `1`, not `3`, so `copyDefaults` cannot mark a pre-v3 profile as already migrated; a documented savedvariables-§1 deviation). `NS.flatDefaults` — alias to `defaults.profile` (the no-AceDB `GetSetting` fallback). `NS.unitDefaults` — alias to `defaults.profile.units.player`, the canonical per-row default source `settings/Appearance.lua` reads from. |
...
|---|------|-------|-------------|
| 25 | `settings/General.lua` | 251 | Two tabs. **Bars** — one `units.<unit>.enabled` row per `NS.Units.LIST` entry (labels "Enable Player/Target/Focus Bar", orders 10 / 20 / 30; `alwaysPerUnit` so `/at get` never tags them with the mirrored note; `onChange` publishes `UNITS` → `APPEARANCE` → `REPAINT`-when-on — the only unit-scoped rows on this page, and the addon's only visibility switch since v4 dropped `hidden`). **Behavior** — `locked` (order 10), `showOnlyInCombat` (order 20, label "Show only in combat"; `onChange` publishes `VISIBILITY`, and `REPAINT` when the bar should show), `throttleWindow` (order 30, `solo`, label "Update throttle (in sec)"). The enable rows are registered **first**, before the three globals, and that is load-bearing rather than tidy: a tab's position in the strip is the position of its group's FIRST registered row. The toggles used to be interleaved with the globals one-for-one, which paired two answers to two different questions on one line. Build closure injects an `InlineButtonPair` ("Reset Position" — delegates to the shared `Helpers.ResetAllPositions` — + "Reset All Settings") under the **Bars** tab via `RenderTabbedSchema`'s `afterGroup` callback, and the **Debug console** checkbox (`Helpers.SessionCheckbox` + `NS.DebugLog:ConsoleCheckbox()`, shows/hides the console window) beside `throttleWindow` via its `pairWith` seam — a `solo` row, so it is alone on its line by construction rather than by the row count above it happening to be odd. Defines the `ABSORBTRACKER_RESET_ALL` `StaticPopupDialog` for the destructive confirm. |
| 26 | `settings/Appearance.lua` | 376 | The one per-unit page, replacing `Bar.lua` + `Border.lua` + `Font.lua` — three pages that each drew their **own** copy of the Unit picker over one piece of session state, so styling the target bar meant picking "Target" three times. `addUnitRows(unit)` generates one row set per `NS.Units.LIST` entry (path `units.<unit>.*`, tagged `unit = unit`), and the array's declaration order **is** the five-tab strip: **Size** (`barWidth` / `barHeight`) — **Bar** (`barTexture` solo, `barColor` + `useClassColorBar`, `barAlpha` solo) — **Background** (`bgTexture` solo, `bgColor` + `useClassColorBg`) — **Border** (`border` / `borderSize`, `borderColor` + `useClassColorBorder`) — **Text** (`font` / `fontSize`, `fontColor` + `useClassColorText`, `fontFlags` solo). Size leads because dimensions are what the page is opened for; the three surface tabs sit in a run because they are read front to back; Text is last because it is set once. All four colored surfaces run the same two lines, `[<surface> Color] [Use Class Color]`, so a reader who has learned one tab has learned four. Target/focus additionally get a group-less `mirror` row (`alwaysPerUnit`, `skipRender` — drawn bespoke by `Helpers.RenderUnitPanel`'s header; group-less so it is neither an empty section heading nor a whole tab holding one invisible row). **No color row carries `disabledIf`** — the swatches used to gray out under their class-color toggle, and that is reversed: setting a color before switching the mode is the normal order of operations and a grayed swatch made it a two-visit job (the alpha channel was live under both modes all along). LSM dropdowns call `NS.Helpers.LSMValues("statusbar" | "border" | "font")`. Build closure calls `Helpers.RenderUnitPanel(ctx, "appearance")` on every `OnShow` (no one-shot guard). |
