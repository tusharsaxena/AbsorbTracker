...
|---|------|-------|-------------|
| 25 | `settings/General.lua` | 387 | Two tabs. **Master controls** (options-ui-§15) — the canonical nine, emitted by `H.MasterControls` from one declaration: `enabled`, `visibility`, `scale`, `alpha`, `locked`, the session-only `state.debugConsole` and `state.testMode`, and the closing **Reset position** / **Reset all settings** button pair the composer returns as its second value. The host supplies only the defaults and each row's `onChange`, attached by **path** so a change to the canonical order upstream cannot move a handler onto the wrong row. **Bars** — one `units.<unit>.enabled` row per `NS.Units.LIST` entry under `subgroup = "Tracked units"` (labels "Enable Player/Target/Focus Bar", orders 10 / 20 / 30; `alwaysPerUnit` so `/at get` never tags them with the mirrored note; `onChange` publishes `UNITS` → `APPEARANCE` → `REPAINT`-when-on), then `throttleWindow` under `subgroup = "Updates"` (order 40). The master block is registered **first**, and that is load-bearing rather than tidy: a tab's position in the strip is the position of its group's FIRST registered row, and §15 requires Master controls to lead. There is no `Behavior` tab any more — lock moved to Master controls, the combat gate became `visibility` there, and the one row left merged into Bars under its own subsection heading. The build closure wires the composer's tail through `RenderTabbedSchema`'s `afterGroup` under `H.MASTER_GROUP` (the group name IS the hook key) and uses **no** `pairWith`: its one user, the bespoke Debug console checkbox, is a schema row now. Also binds `state.debugConsole` to `NS.DebugLog:ConsoleCheckbox()` and `state.testMode` to `NS.State.testMode` (refusing a start in combat) through `NS.RegisterSessionSetting`, and defines the `ABSORBTRACKER_RESET_ALL` `StaticPopupDialog` for the destructive confirm. |
