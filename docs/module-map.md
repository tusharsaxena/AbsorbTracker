...
| 9 | `core/Units.lua` | 138 | `NS.Units` — the only file that reads `db.profile.units` for appearance. `LIST`/`LABEL` (player/target/focus identity), `APPEARANCE_KEYS` (the 19 per-unit keys; `tests/test_units.lua` asserts the list and the schema's per-unit rows agree in **both** directions, because a key in one and not the other is silently half-broken), `Config(unit)`, `IsEnabled(unit)`, `IsMirrored(unit)` (always false for player), `SourceUnit(unit)`, `Get(unit, key)` (the mirror-resolved read path), `Set(unit, key, value)` (writes the unit's own config, never mirror-resolved — published without a production caller as the write half of the `Get` seam; `/at set` does **not** come through here), `DeepCopy(v)` (the addon's ONE recursive table copy — `core/Database.lua` uses it too), `Position(unit)` / `SetPosition(unit, pos)` (never mirrored), and `CopyFromPlayer(unit)` (one-shot deep-copy of the 19 appearance keys, then clears `mirror`; does not copy `position` or `enabled`). |
...
| 31 | `tests/_kit/` | 2405 | The **shared headless harness**, vendored from `LibKa0s/testkit` (kit revision 16) and kept byte-identical to it (`diff -r` is part of the green gate). `framework.lua` (913) is the case registry, the assertions and the runner; `loader.lua` (169) `loadfile` + `setfenv`s each chunk into an environment where WoW globals resolve to the mock set (falling back to `_G`) and writes land in `_G`, plus `tocFiles()` for TOC-derived load lists and `xmlFiles()` for the vendored library half the TOC reaches through an XML; `mock_base.lua` (777) is the universal half of the WoW-API mock, `AceGUI:Release` and its `__released` recorder included; `vendor_sync.lua` (380) is the shared vendor-drift comparator `tests/test_vendor_sync.lua` drives, and also registers the runner-mode case (`run-automated-tests.sh` recorded `100755`); `test_eol.lua` (166) is a **suite** the kit ships rather than a helper — `tests/run.lua` declares it with an explicit `dir`, so every consumer inherits the line-ending gate instead of re-typing it. It lives under `tests/` rather than `libs/` precisely so the `- tests` entry `.pkgmeta` already carries excludes it from the zip. |
