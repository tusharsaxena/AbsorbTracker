...
| 2a | `core/MediaSetup.lua` | 144 | The `LibKa0s-Media-1.0` seam. Publishes `NS.Icon(name)` and `NS.MediaFont(name)` over `LibStub("LibKa0s-Media-1.0", true)`, each passing this addon's own **folder name** — the file's first vararg, which is the one thing a vendored library cannot work out for itself, since a texture path is absolute from `Interface\AddOns\` and a copy cannot know which folder it was copied into. Paths come back **extensionless** (`…\media\icons\close`, never `close.tga`; the client appends it, and a path carrying the extension draws nothing and raises nothing). `nil` is a real answer twice over — no library, or no such name — and is never routed around by building a path with concatenation. Ends with the one `Media.RegisterLSM(addonName)` call, at **file load** rather than at `OnInitialize` where this addon's hand-rolled `LSM:Register("font", "JetBrains Mono", …)` used to sit: LibSharedMedia is vendored under `libs/` and has already run, while `defaults/Profile.lua` names faces at load time, so deferring leaves a window where a shipped default names a face LSM has never heard of. It also registers the shared statusbar textures, which is why Ka0s bar textures appear in the Bar page's dropdown. **TOC position is load-bearing** — before `Constants.lua`; `tests/test_loadorder.lua` pins it and the TOC line carries the reason. |
