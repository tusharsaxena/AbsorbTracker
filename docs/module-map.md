...
English-only — a ratified row in `ARCHITECTURE.md`'s Documented deviations register (localization-§1) rather than a backlog item. The metatable returns the key itself, so untranslated strings work and a missing key never errors. **One** string is wrapped: `settings/Slash.lua`'s disabled-verb refusal (slash-commands-§2), the single line a disabled addon says. Everything else — labels, tooltips, the rest of the slash output, the reset-confirm popup — is still hardcoded English, and the seam is in place for a future pass.
...
|---|------|-------|----------------|
| 1 | `locales/enUS.lua` | 27 | Canonical locale. Sets `NS.L = setmetatable({}, { __index = function(_, k) return k end })` so a missing key returns itself and never errors. English-only, recorded in the deviations register (localization-§1); one key is live — `settings/Slash.lua`'s disabled-verb refusal — and `enUS.lua` carries no key nothing reads (localization-§3). |
