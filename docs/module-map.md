...
| 39 | `tests/test_timer.lua` | 175 | `NS.RequestRepaint` coalescing + `throttleWindow` delay, `OnAbsorbChanged` / `OnMaxHealthChanged` (player/target/focus) + `OnEnterWorld` + `OnUnitSwap` repaint wiring (via the bus). |
