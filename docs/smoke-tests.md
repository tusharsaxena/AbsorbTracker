...
84. **Cancel is always live and always red.** The **Cancel run** row is muted red and clickable at every point — before a run, mid-experiment, and after `finish`. Mid-run, click it → chat shows `perf run CANCELLED`, the bars come back if Experiment B had suspended them, the panel closes, and **nothing is added to `AbsorbTrackerPerfDB`**. Start a fresh run → the panel is back at step one with no carry-over. `/at perf cancel` with no run → `no perf run to cancel`.
...
**Pass criteria:** all 86 checks pass with **no Lua errors**, the `[AT]` prefix on every chat line,
