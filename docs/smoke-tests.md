...
14e. **Test mode checkbox (preview-mode).** On **Master controls**, **Test mode** sits alone on the line under **[Lock frame | Debug console]**; its tooltip names `/at test` and says combat turns it off. Lock the bars, enable the Target bar, clear your target, and with no shield up tick **Test mode** → every enabled bar, the Target bar included, shows a partial placeholder fill reading `Absorb`, and the bars still cannot be dragged. Set **General visibility** to *Never* → the bars stay up while Test mode is on. Untick → the placeholder goes and live data returns; with *Never* still set, every bar hides (set it back to *Always*). Tick it again and pull a training dummy → the moment combat starts, chat prints `[AT] Test mode off — combat started` and the bars show live data; with the panel open, or reopened after the fight, the box reads unticked. **The verb is the same switch:** with the panel open, `/at test` → `[AT] Test mode on …`, the box ticks itself and the placeholders show; `/at test` again → `[AT] Test mode off`, the box unticks; `/at test on` twice stays on, `/at test off` twice stays off. In combat, `/at test` (or ticking the box) → `[AT] Cannot start test mode during combat` and nothing changes. Tick it and `/reload` → off. Tick it and **Reset all settings** → off.
...
34. `/at test 50000` with every bar disabled → `Every bar is disabled; run /at toggle to turn them on…`; no hold window is armed. `/at test wibble` → a usage line naming both forms, and neither test mode nor a hold starts.
...
68. **Unit labels appear only while unlocked.** `/at unlock` → a small label sits just above each visible bar reading **Player** / **Target** / **Focus**, in that bar's own font face at a fixed small size. `/at lock` → every label disappears. Enable target/focus and confirm each label names the bar it sits on. Change a bar's Font Face on Appearance → *Text* while unlocked → its label follows the new face; change its Font Size → the label does **not** grow (fixed 10pt by design). The label is a drag affordance with no schema row, so it appears on no settings page and in no `/at list` output.
...
- **`AbbreviateNumbers`** — the bar's value text (`modules/Display.lua:399`), the `/at test` line
  (`settings/Slash.lua:296`, `:300`) and three debug lines (`core/AbsorbTracker.lua:176`, `:178`,
