...
81. **`report` and `dump`.** Close the debug console, then click **Report** in the panel → **the console opens by itself** and the summary is in it. Click **JSON Dump** → one JSON line starting `{"buckets":...`, carrying `"schema":1` and `"source":"ingame"`, appears **in the console** (not in a popup). Use the console's own **Copy** button → it pastes as valid JSON.
