...
| `tests/test_vendor_sync.lua` | `libs/LibKa0s/` and `tests/_kit/` are exactly what LibKa0s published at the tag `README.md`'s provenance line names. Compared against the **tag**, not upstream's working tree; the provenance line is an input, not a constant; a missing sibling checkout **skips with its reason** rather than passing. The comparator is `tests/_kit/vendor_sync.lua` — part of the payload it checks |
