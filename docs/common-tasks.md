...
| `tests/test_disabled.lua` | The `slash-commands-§7` conformance suite: disabling through `NS.SetByPath` leaves **nothing registered** (events, unit events, bus messages), no timer armed and every bar hidden, all read from the kit's recording mocks rather than a handler's return value; baseline events fired anyway write, print and show nothing; every reserved verb and the bare `/at` answer while only feature verbs refuse; the launcher's left click opening the panel and its menu graying Locked while Enabled stays live; re-enable rebuilds from current settings; and the `disabled` / `perf` holds in both orders |
