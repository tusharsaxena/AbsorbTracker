...
- **Ticker drives visual updates, not events.** `UNIT_ABSORB_AMOUNT_CHANGED` only records a debug line; the periodic AceTimer repeating timer is the source of truth for `UpdateAbsorbBar` calls. `modules/Timer.lua` schedules it via `NS.addon:ScheduleRepeatingTimer` and tears it down with `NS.addon:CancelTimer` (an interval-change guard avoids needless restarts). This decouples the ~Hz event flow from the user-configurable update interval.
