...
- **The three master toggles stay global.** `locked`, `showOnlyInCombat` and `throttleWindow` govern all three bars; appearance, position and `enabled` are per-unit. (A fourth, `hidden`, was removed in schema v4 — the per-unit `enabled` flags replaced it.)
...
- **Combat lockdown gate on `/at config` (refuse, options-ui-§2).** `Settings.OpenToCategory` is protected; opening any settings subcategory during combat would taint the panel. `NS.OpenOptionsPanel` **refuses** while `InCombatLockdown()` is true — it prints a grey `[AT]` notice and returns, and does not defer-and-replay the open.
- **Visibility is a per-unit ladder, `enabled` first.** `NS.ShouldShowBar(unit)` starts from that unit's own `enabled` flag (there is no master `hidden` toggle — v4 removed it), then applies the `showOnlyInCombat` gate, then `UnitExists` for target/focus. When `showOnlyInCombat` is on, an enabled bar shows only while `UnitAffectingCombat("player")` is true (actual player combat — **not** `InCombatLockdown()`, which lags the `PLAYER_REGEN_DISABLED` edge; see [midnight-quirks.md](./midnight-quirks.md)). `NS.ApplyVisibility()` applies the result. `UpdateBarAppearance` and `UpdateAbsorbBar` route every paint through these two functions rather than reading `hidden` directly.
