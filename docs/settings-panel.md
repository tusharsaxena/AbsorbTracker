...
The **Debug console** checkbox toggles the **visibility of the console window only** — the same as the bare `/at debug`. It deliberately does **not** change the debug logging flag (`NS.State.debug`); that stays on `/at debug on|off` and the window's own header toggle. It **is** a schema row now, at the path `state.debugConsole` and carrying `sessionOnly = true` (options-ui-§15) — where it used to be a bespoke `SessionCheckbox` injected through `pairWith`. Its value still never reaches the profile: `core/Data.lua` keeps a small **session-settings registry**, and `settings/General.lua` binds that path to the `{ get, set }` pair `NS.DebugLog:ConsoleCheckbox()` already answers, so `NS.GetSetting` / `NS.SetSetting` route it to the window rather than to `db.profile`. What being a row buys is reach: `/at get state.debugConsole` and `/at set state.debugConsole true` work, where no CLI verb could see the bespoke checkbox. `settings/Schema.lua`'s validator exempts `sessionOnly` rows from the "path must resolve against `defaults.profile`" check for the same reason — not being in the profile is the point of them. Whenever the window's visibility changes by any route — this checkbox, bare `/at debug`, the header Close button, or Esc (`UISpecialFrames`) — the console frame's `OnShow`/`OnHide` hooks fire `LibKa0s-DebugLog-1.0`'s `onVisibilityChanged` descriptor hook, which `core/DebugLogSetup.lua` points at `NS.Helpers.RefreshAllPanels` — so an open settings panel's checkbox stays in sync. `ConsoleCheckbox()` itself returns a plain `{ label, tooltip, get, set }` table: a data contract the library hands over and this addon assembles into a widget, so neither side reaches into the other.
