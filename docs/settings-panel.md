...
- Optional **Defaults** button (width `DEFAULTS_W = 110`) at TOPRIGHT — General / Bar / Border / Font opt in via `opts.defaultsButton = true`. This button is the **only** way to revert a whole page: it calls `Helpers.RestoreDefaults(pageKey, ctx)`, which reverts that page's rows across all three units. `/at reset` takes a single setting path, not a page name. About page and Profiles deliberately omit it (about page has no settings; Profiles has its own destructive controls inside the AceDBOptions UI).
