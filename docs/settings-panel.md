...
| `libs/LibKa0s/OptionsWidgets.lua` | `RenderField` (dispatches by `row.type`) + `RenderRows` (two-column layout over an explicit row list, skipping `skipRender` rows) + the thin `RenderSchema(ctx, pageKey, ...)` wrapper + `RenderTabbedSchema` and its parts, `TabStrip` / `PageBanner` / `SetChromeHeight` (the chrome band, options-ui-§13/§14) + the five widget makers (CheckBox / Slider / Dropdown / EditBox / ColorPicker) + `Section` / `AddSpacer` / `AttachTooltip` / `InlineButtonPair` / `SessionCheckbox`. |
...
| `settings/UnitPanel.lua` | The two pieces that did not generalize: `Helpers.RenderUnitPanel(ctx, pageKey)` (the Appearance page's Unit banner + mirror header + tab strip, full rebuild per call) and `Helpers.ResetAllPositions()`. |
...
So the stub publishes every member a page file touches at load time. Measured, one member at a time, against `#NS.Schema` over a library-absent load: `LSMValues` is the **only** load-time member, and `RestoreAllDefaults` (the `StaticPopup` body in `settings/General.lua`, a table literal evaluated at load) is kept anyway because the call it answers is a recovery path — `/at resetall` for a user whose panel will not open. The layout constants `ROW_VSPACER`, `SECTION_HEADING_H`, `BUTTON_PAIR_REL`, `CHROME_GAP`, `TAB_H` and `BANNER_H` are **not** there: they are copies of `lib.LAYOUT`'s values, nothing in this addon reads them, and a host copy of a library constant is the copy that goes stale (`tests/test_optionssetup.lua` pins their absence, `tests/test_surface_parity.lua` exempts them by name). The four **callable** members of the chrome band — `SetChromeHeight`, `TabStrip`, `PageBanner`, `RenderTabbedSchema` — *are* there, as no-ops, because a page builder reaches them. `RestoreAllDefaults` is real, not a no-op: the rows are the schema's and the schema loaded fine, so a reset still works with no panel at all. Losing the panel is survivable; losing the reset would not be. Everything else is reached from `CreateOptionsPanel` or from a page builder, both user-triggered, so those are no-ops, and `NS.CreateOptionsPanel` / `NS.OpenOptionsPanel` print one honest MISSING line.
...
picker belongs in the page banner and that there must be exactly **one** of it. No path moved with
...
The parent and every sub-page register as **canvas-layout categories**: a custom Blizzard `Frame` is registered with `Settings.RegisterCanvasLayoutCategory` (parent) / `Settings.RegisterCanvasLayoutSubcategory` (each sub-page) and Blizzard renders it in its own settings panel slot. The schema-driven sub-pages (General / Appearance) lay out their schema rows as **AceGUI widgets** (`CheckBox` / `Slider` / `Dropdown` / `ColorPicker`) inside an AceGUI `ScrollFrame` parented to the page's `body` frame, under a pinned **chrome band** holding the page's tab strip and, on Appearance, its banner.

**Appearance is per-unit; General and About are not.** Appearance renders through `Helpers.RenderUnitPanel(ctx, pageKey)` (`settings/UnitPanel.lua`), which draws the **Unit** banner (Player/Target/Focus) in the chrome and filters the rows to the selected unit. General has no Unit picker — its three globals are unit-agnostic, and its three `units.<unit>.enabled` toggles are all rendered at once rather than filtered — and About has no settings at all.
...
The per-unit page renderer, and one of the two things `settings/UnitPanel.lua` keeps. It did not generalize: it reads `NS.Units` and the mirror partition, which no other Ka0s addon has. Everything it stands on — `ClearScroll`, `EnsureScroll`, `RenderGrid`, `RenderRows`, `PageBanner`, `TabStrip`, `AttachTooltip` — is the library's, reached through the same `NS.Helpers` table.

On every call (first `OnShow`, a banner switch, a tab click, a mirror-checkbox toggle, or a Copy click) it does a **full rebuild**: `Helpers.ClearScroll(ctx)` releases every AceGUI child and resets the section-heading tracker + `ctx.refreshers`, then draws three bands, top to bottom:

1. **The banner** (chrome, options-ui-§14) — always. `Helpers.PageBanner` with a **Unit** dropdown listing `NS.Units.LABEL` in `NS.Units.LIST` order. Changing it sets `ctx.unit` and re-runs `RenderUnitPanel`. It is parked on `ctx.__bannerWidget` so a suite can drive the selection the way a click would; the library keeps its own chrome widgets private. It is drawn **before** the strip, because `PageBanner` records its share of the band in `ctx.__bannerHeight` and `TabStrip` adds that to the rows it reserves for itself — the other way round and the tabs would draw on top of the picker.
2. **The mirror header** (scroll) — target and focus only, never the player, which is the mirror source. A **"Use same styling as Player"** checkbox (writes `units.<unit>.mirror` via `NS.SetByPath`, re-renders on change) side-by-side with a **"Copy styling from Player"** button (`NS.Units.CopyFromPlayer(unit)` — a one-shot deep-copy of the appearance keys that also clears `mirror`, followed by publishing `NS.MSG.APPEARANCE` and a re-render). While mirrored, a hint label reads *"Linked to Player — uncheck to customize."* Both are **bespoke**: neither has a schema path, so neither can be a row, which is the case options-ui-§13 exempts from its two-controls-per-tab rule rather than loosening it.
3. **The tab strip** (chrome, options-ui-§13) and the active tab's rows — only while the unit is **not** mirrored. The tabs are one per distinct `group` among the unit's rows, in registration order (`Helpers.__partitionTabs`); a stale `ctx.activeTab` heals to the first rather than rendering a blank page under a full strip. Every appearance row on this page is hidden by the mirror, so a mirrored unit would otherwise get five tabs of nothing to click through — the hint line takes their place instead. The mirror row itself carries `skipRender` and **no group**, so it belongs to no tab and `RenderRows` leaves it to the header.

The strip is driven with `Helpers.TabStrip` directly rather than by handing the page to `Helpers.RenderTabbedSchema`, and that is the one interesting choice here. `RenderTabbedSchema` owns the whole page body: its tab click is `ClearScroll` + a re-render of *itself*, which would drop the banner and the mirror header this page draws above the rows. A tab click here re-renders the whole page instead — the same path the banner and the mirror checkbox already take.

It then registers one final **header refresher** on `ctx.refreshers`. **This is not decoration.** The mirror checkbox and the Copy button in band 2 are built inline, not through `Helpers.RenderField`, so neither registers a refresher of its own — without this one, `Helpers.RefreshAllPanels` structurally could not update them, and nothing re-ran the mirrored/unmirrored partition that decides whether band 3 is drawn at all. `Helpers.RestoreDefaults("appearance", ctx)` resets `units.focus.mirror` to its default `true` and then runs only the refreshers, so the header checkbox would still read unchecked and the appearance rows would stay on screen over a now-mirrored unit (same after `/at set units.focus.mirror true`, and after a profile switch). It self-corrected on the next `OnShow`, which is what made it present as "the panel lied to me once".
...
Behavior   [Lock Position]       [Show only in combat]
           [Update throttle]     [Debug console]        <- pairWith partner
```
...
The three enable toggles used to be **interleaved** with the globals one for one
(`[Enable Player Bar] [Lock Position]`), which put two answers to two different questions on one
line. Reading across a line is supposed to compare two answers to *one* question, so the toggles now
run together and the globals run together.

**`throttleWindow` is the Debug console's `pairWith` host, and it is `solo`.** A `pairWith` partner
attaches only to a row that is alone on its line, and `solo = true` guarantees that by construction.
The old host — Enable Focus Bar — qualified only because the five rows above it happened to pair off
as 2 + 2 + 1, an accident any added row would have broken silently.
