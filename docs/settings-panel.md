...
So the stub publishes every member a page file touches at load time. Measured, one member at a time, against `#NS.Schema` over a library-absent load, that set is **six**: `LSMValues`, plus the five schema **composers** — `ColorPair`, `FontGroup`, `BorderGroup`, `BarGroup` and `MasterControls` — which `settings/General.lua` and `settings/Appearance.lua` call inside `NS.RegisterSchemaRows`, again at file load. `RestoreAllDefaults` (the `StaticPopup` body in `settings/General.lua`, a table literal evaluated at load) is kept anyway because the call it answers is a recovery path — `/at resetall` for a user whose panel will not open.

The stub's composers draw a line, and the line is what keeps them from being the copy of the library this file forbids two paragraphs down. They reproduce the **stored surface** and only that: one row per canonical leaf, at the path the live composer derives (so `prefix` and `keys` are honored identically), carrying its type and the caller's `default`. Master controls' optional **Test mode** and **Minimap button** leaves are mirrored the same way: present only when the host names `testModePath` / `minimapPath`, at that path verbatim. The minimap leaf additionally carries `default = true`, its own SHOWN sense, because that is the one canonical default the live composer hard-codes rather than reading off the caller's `defaults`. They reproduce **none** of the labels, tooltips, ranges, media sources, `startsLine`, `hasAlpha` or `classColorSource` the live composers emit — every one of those is read by a widget or by the library's CLI, and a library-less build has neither (`settings/Slash.lua`'s own stub answers every schema verb with "unavailable"). `tests/test_perf.lua` compares the whole **path set** between the two loads, not just the row count, so a stub composer that drops a leaf or honors `keys` differently fails loudly; `tests/test_optionssetup.lua` names the members and pins the line. The layout constants `ROW_VSPACER`, `SECTION_HEADING_H`, `BUTTON_PAIR_REL`, `CHROME_GAP`, `TAB_H` and `BANNER_H` are **not** there: they are copies of `lib.LAYOUT`'s values, and the only ones this addon reads (`ROW_VSPACER`, `SECTION_HEADING_H`, `BUTTON_PAIR_REL`, `BANNER_H`) are read from inside a render a library-less build never reaches, and a host copy of a library constant is the copy that goes stale (`tests/test_optionssetup.lua` pins their absence, `tests/test_surface_parity.lua` exempts them by name). The four **callable** members of the chrome band — `SetChromeHeight`, `TabStrip`, `PageHeader`, `RenderTabbedSchema` — *are* there, as no-ops, because a page builder reaches them (`PageBanner` is not: the Appearance page's one chrome block is a `PageHeader`, so nothing in this addon calls the banner). `RestoreAllDefaults` is real, not a no-op: the rows are the schema's and the schema loaded fine, so a reset still works with no panel at all. Losing the panel is survivable; losing the reset would not be. Everything else is reached from `CreateOptionsPanel` or from a page builder, both user-triggered, so those are no-ops, and `NS.CreateOptionsPanel` / `NS.OpenOptionsPanel` print one honest MISSING line.
