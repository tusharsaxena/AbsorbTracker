...
| Page | Tabs | Covers |
|------|------|--------|
| General | **Master controls**, **Bars** | *Master controls*: turn the whole addon off, choose when it shows at all (always / only in combat / only out of combat / never), scale and fade every bar together, lock them, show the debug console, turn test mode on, and the two reset buttons. *Bars*: turn each bar on or off (Player / Target / Focus), and the repaint throttle — how fast the bars may redraw during a burst of changes. |
...
**Appearance is per-unit; General and About are not.** Appearance renders through `Helpers.RenderUnitPanel(ctx, pageKey)` (`settings/UnitPanel.lua`), which draws the **Unit** picker (Player/Target/Focus) in its chrome block and filters the rows to the selected unit. General has no Unit picker — its seven unit-agnostic rows (`enabled`, `visibility`, `scale`, `alpha`, `locked`, `state.debugConsole`, `throttleWindow`) apply to the addon as a whole, and its three `units.<unit>.enabled` toggles are all rendered at once rather than filtered — and About has no settings at all.
...
bar — so all nine are here, Test mode included: every addon with a positionable display ships a test
mode (preview-mode).
...
The **Test mode** checkbox is the addon's test mode (preview-mode), composed from `testModePath` and drawn on its own line under Lock frame / Debug console. It is `sessionOnly` and bound the way the console row is: `settings/General.lua` registers `state.testMode` with `NS.RegisterSessionSetting`, over `NS.State.testMode`, so it never reaches the profile and is off after every `/reload`. On, every enabled bar paints the placeholder fill whether or not the bars are locked, live repaints stand down, and a target or focus bar shows with no target or focus and under any **General visibility** (`NS.InPreview` and the test-mode rung of `NS.ShouldShowBar`, in `modules/Display.lua`). It never touches the lock, so the bars are only draggable if Lock frame is off. Combat ends it: `OnEnterCombat` writes it off through `NS.SetByPath`, prints `Test mode off — combat started` and calls `NS.RefreshOptionsPanel`, so an open box unticks. A start in combat is refused with a chat line and the box stays unticked. The row's `default = false`, passed through the composer's `defaults`, is what lets Reset all settings and the page's Defaults button end the mode. Its onChange is the lock's three steps (drop any `/at test` hold, `APPEARANCE`, `REPAINT`), run only when the write actually moved the mode. The tooltip is this addon's rather than the composer's generic one. Bare `/at test` toggles the same state from chat and `/at test on|off` sets it, through the same `NS.SetByPath`, so the box follows; `/at test <value> [secs]` is still the one-shot fake value held for a few seconds.
