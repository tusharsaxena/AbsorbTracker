...
1. **Logo.** `spec.logo` is `NS.Constants.LOGO_PATH` (`media/logos/absorbracker.logo.v2.tga`), drawn at `LAYOUT.LANDING_LOGO` (300×300), anchored TOPLEFT inside a full-width SimpleGroup.
2. **TOC `Notes` blurb** — full-width `Label` with `GameFontHighlight`, left-justified. `spec.notes` is a **function**, because the library resolves it at render time and the TOC field is not readable when the spec is declared at file scope. The Notes string is read through `NS.Compat.GetAddOnMetadata` (Ka0s standard `compat` — the single deprecated-API shim).
