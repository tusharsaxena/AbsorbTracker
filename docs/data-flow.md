...
| `PLAYER_REGEN_ENABLED` (leave combat) | `addon:OnLeaveCombat` | Publishes `VISIBILITY` + `REPAINT` — nothing else. Per options-ui-§2 the settings panel **refuses** to open in combat (the `InCombatLockdown()` check and the grey `STRINGS.COMBAT_REFUSED` notice are `libs/LibKa0s/Options.lua`'s, printed through the descriptor's `print` so the line still carries `[AT]`; reached via `NS.OpenOptionsPanel` in `settings/OptionsSetup.lua`) rather than deferring, so there is no combat-deferred `/at config` for `OnLeaveCombat` to replay. It is the sole handler of `PLAYER_REGEN_ENABLED`, and its only job is re-evaluating the `showOnlyInCombat` visibility gate and repainting. |
