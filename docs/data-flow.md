...
| `PLAYER_REGEN_DISABLED` (enter combat) | `addon:OnEnterCombat` | Publishes `VISIBILITY` + `REPAINT` — re-evaluates the `visibility` gate so any eligible bar appears (and repaints fresh) the moment combat starts. |
