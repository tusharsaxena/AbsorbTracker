...
1. the addon-wide `enabled` flag (options-ui-§15's Master controls row), then the per-unit `NS.Units.IsEnabled(unit)` flag (target/focus ship `false`) — the two are different settings and §15 forbids conflating them: one is *whether the addon draws at all*, the three are *which bars exist*. Neither is the pre-v4 `hidden` global returning; that key was dropped because nothing in the UI could clear it. Past this step, being **unlocked** answers *shown* and skips steps 2 and 3, so a bar can be seen and placed with no target and under any `visibility` (this rung was test mode's until options-ui-§15 exempted the addon from that row — the lock is the one preview switch now),
