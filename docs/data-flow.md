...
3. **The account-wide ladder** — one loop over the file-local `SCHEMA_STEPS` array, which is the ordered list of `{ to = N, apply = function(profile) ... end }` rows. For each row whose `to` is above the stored `g.schemaVersion`, the loop runs `apply`, logs one `v<old> → v<to>` line, then stamps `g.schemaVersion = to`. `apply` runs *before* the stamp line, so a step's own output still precedes its version line, and the ladder climbs one version at a time rather than jumping. The four rows today:
   - **v2**: retires the dead `profile.updateInterval` key (the repaint path moved from a poll ticker to the event-driven `throttleWindow` scheduler).
