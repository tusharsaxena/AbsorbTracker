...
2. **Backfill** (unconditional, idempotent): walks `NS.defaults.profile` and copies any key still missing from `db.profile` — both the six flat globals (`enabled`, `visibility`, `scale`, `alpha`, `locked`, `throttleWindow`) plus the per-profile `schemaVersion` stamp and, per unit in `NS.Units.LIST`, any missing per-unit appearance key (deep-copying table defaults so a saved-variable mutation can't reach back into the defaults). Because it only fills absent keys, it is a no-op on the second run — this step absorbs the legacy pre-AceDB flat-SavedVariables shape and the no-AceDB fallback.
3. **The account-wide ladder** — one loop over the file-local `SCHEMA_STEPS` array, which is the ordered list of `{ to = N, apply = function(profile) ... end }` rows. The stored stamp starts from `0` (the shipped default, and what an absent stamp reads as — savedvariables-§1), and the runner's target is `NS.SCHEMA_VERSION`, derived from the last row's `to` rather than written a second time. For each row whose `to` is above the stored `g.schemaVersion`, the loop runs `apply` under `pcall`, logs one `v<old> → v<to>` line, then stamps `g.schemaVersion = to`. The runner owns the stamp: a step never writes it, and a step that **raises** leaves it at the last completed step, logs a `[Migrate] … failed` line, prints one chat line (`Settings upgrade stopped at v<old> -> v<to>; saved settings were left as they were`) and stops the ladder, so the next load retries that step instead of skipping it. `apply` runs *before* the stamp line, so a step's own output still precedes its version line, and the ladder climbs one version at a time rather than jumping. The four rows today:
   - **v2**: retires the dead `updateInterval` key (the repaint path moved from a poll ticker to the event-driven `throttleWindow` scheduler) from **every** profile in the store. It used to clear the active profile only, which under the account-wide stamp left the key on every other stored profile for good.
   - **v3**: stamp-only. This is the DB-wide marker; the *per-profile* stamps were already written in step 1, so there is nothing account-wide left to do.
   - **v4**: drops the dead `hidden` master toggle from **every** profile in the store via `dropKeyEverywhere` (same all-profiles reasoning as the v3 lift — a key left on an inactive profile returns the moment the user switches to it). The per-unit `enabled` flags replaced it, so a surviving `hidden = true` would suppress every bar with no UI left to clear it.
   - **v5**: maps the `showOnlyInCombat` **boolean** onto the four-value `visibility` **dropdown** (`true` → `"inCombat"`, `false` → `"always"`), on every profile in the store, then deletes the boolean. A stored-value *type* change takes a ladder step in the same change as the row (options-ui-§15); changing the row's type alone would meet a stored `true` where the panel expects one of four strings and lose a setting the player already made, silently. It runs **after** the backfill above, and the old value deliberately wins over the `"always"` the backfill just seeded.

**Adding a schema version is one new row appended to `SCHEMA_STEPS`**, not another `if g.schemaVersion < N` arm — the loop already owns the ordering, the logging and the stamp.

Every ladder step is idempotent and logs one `[Migrate]` line only when it actually fires.
...
The two `UNIT_*` events go through **one private `CreateFrame` frame per unit** with `RegisterUnitEvent`, not AceEvent — the events-frames-taint-§1 unit-filter carve-out ([ARCHITECTURE.md → Event Subscriptions](./ARCHITECTURE.md#the-per-unit-frames-are-the-unit-filter-carve-out)). These events fire for *every* unit the client knows about; AceEvent-3.0 shares one frame and cannot `RegisterUnitEvent`, so a plain `RegisterEvent` would pay a C→Lua dispatch per unit only to discard all but ours. The private frames filter at the C layer instead. **A frame each, rather than packing tokens:** `RegisterUnitEvent` filters at most two unit tokens per registration, so three units could never share one frame anyway — and with a frame each, enabling or disabling a bar is a registration change on that unit's own frame, leaving a **disabled unit registered for nothing at all**. `PLAYER_TARGET_CHANGED` / `PLAYER_FOCUS_CHANGED` stay on AceEvent but are gated on the same flag by the same function, which is where the saving actually lands (they fire on every swap in ordinary play). `PLAYER_ENTERING_WORLD` and the combat pair are unconditional AceEvent subscriptions. **Every one of those seven registrations is pcalled** (events-frames-taint-§1): the file-local `safeRegister` / `safeRegisterUnit` in `core/AbsorbTracker.lua` call `NS.SafeRegisterEvent` / `NS.SafeRegisterUnitEvent` (`LibKa0s-Core-1.0` minor 8, bound by `core/CoreSetup.lua`), which front-gate on `C_EventUtils.IsEventValid` where present and `pcall` the registration. The client raises on an unknown event name, so without the helper one name a patch retires would take every registration after it down; through it, the refused name costs only itself and lands once in `NS.State.rejectedEvents`, which `/at debug events` prints and the `[Init]` summary counts. The `if NS.CreateOptionsPanel then ... end` guard is the [forward-reference pattern](./module-map.md#forward-references) — in practice the call always succeeds because all files load synchronously before `OnEnable` fires, but the nil-check keeps the load-order coupling soft.

### The latch gate, and what the registrations record

**Everything between `GetLSM` and `CreateOptionsPanel` in the diagram above runs only while the
lifecycle latch is up.** `OnEnable` calls **`NS.SyncEnabledHold()`** straight after `GetLSM`, and an
addon that comes up disabled publishes nothing and registers nothing at all: it does not register
five events and three unit frames only to tear them down in the same turn
([lifecycle.md](./lifecycle.md)). `CreateOptionsPanel` sits outside that gate, because the settings
registration is setup rather than a feature. The three publishes reach `RestoreBarPosition` /
`UpdateBarAppearance` (Display) and `RequestRepaint` (Timer), so the login paint lands one
`throttleWindow` later, not synchronously.

The per-unit frames are built once, while `addon.__unitEventFrames` is nil, and reused: every later
call (the `UNITS` message, `StandUp`) changes only their registrations. Before per-unit gating this
was two frames, `"player", "target"` on one and `"focus"` on the other, packed against
`RegisterUnitEvent`'s two-token cap. `StandUp` inherits all seven pcalled registrations through
`RegisterLifecycleEvents` and `SyncUnitEventFrames`. A refused event name is logged under the
`Events` debug tag, appends `, rejected events: <n>` to the `[Init]` summary when the list is
non-empty, and prints on `/at debug events`. With the library absent, `core/CoreSetup.lua`'s stub
keeps the `pcall` and the list and drops the `IsEventValid` front gate. `tests/test_events.lua`
drives both halves through the kit's `__badEvents`.
...
cli:CliSet(rest) → lib.ParseValue        local set(row, value)
            ...
            └────────────────┬─────────────────────────┘
                  ...
                  ┌──────────┴───────────┐
                  ▼                      ▼
       store (path, v)          onChange / announce   -- publishes, never calls Display directly
       NS.SetPath(db.profile,   default:  bus ▶ APPEARANCE
         ...
         same as flat ones
                   ...
                   (slash path → NS.RefreshOptionsPanel;
                    panel set() → Helpers.RefreshScalars
                    so paired controls re-sync -- every refresher
                    re-pulls its row's value into its widget, and
                    only the slash path is entitled to redraw)
                             │
                             ▼
...
The slash and panel paths converge on `NS.SetByPath` (`settings/Schema.lua`) — the single write seam (the `LibKa0s-Schema-1.0` instance's `Set`) that stores the value, then runs the row's `onChange` and the announce. A reset to a row's default (`/at reset`, a page's Defaults button) reaches the same seam through `NS.ApplyDefault`, which only builds the copied default and hands it to `SetByPath`, so a single-row reset logs the same `[Set]` line as any other write. A bulk act — a page's Defaults, Reset All, **Copy styling from Player** — runs inside the `NS.Bulk` bracket instead: every row is still written and still fires its `onChange`, but the log collapses to one `[Set] <act> <scope>: N rows` line, N being the rows whose stored value changed (debug-logging-§10). The seam runs `row.onChange` and then the descriptor's `announce`, which, for a row with no `onChange`, publishes `NS.MSG.APPEARANCE` on the bus (so the write path signals the display module instead of calling `UpdateBarAppearance` across the module boundary). The panel's local `set(row, value)` is library code — `libs/LibKa0s/OptionsWidgets.lua`, file-local to the widget makers — and reaches `SetByPath` through the `set` closure the addon declares in its options descriptor (`settings/OptionsSetup.lua`), then calls `Helpers.RefreshScalars` -- the value tier, which re-runs `ctx.refreshers` in place on a page that is on screen and never rebuilds it, so a checkbox click cannot release the widget whose callback is still on the stack; the slash path (LibKa0s-Slash-1.0's `CliSet`, through the `set` closure in the descriptor `settings/Slash.lua` hands it) calls `SetByPath` then `NS.RefreshOptionsPanel` (which itself routes to `Helpers.RefreshAllPanels`). Color getters resolve `useClassColor*` at call time, so no explicit "switch class color on" wiring is needed — the next paint reads the current toggle state and produces the right color.
...
`NS.ShouldShowBar(unit)` (`modules/Display.lua`) is the single source of truth for whether a given unit's bar is on screen — a four-step ladder, first `false` wins (ahead of it sits a step 0 that is not part of the schema: `NS.IsStoodDown()`, the lifecycle latch from `core/Lifecycle.lua`, which is down while either the `disabled` hold — the addon-wide `enabled` row — or a perf run's `perf` hold is taken). Step 0 is **not** the stand-down itself: by the time it answers no, `StandDown` has already unregistered every event, bus subscription and timer; the rung only keeps a combat transition, a target swap or a settings change from re-showing a bar behind the latch's back ([lifecycle.md](./lifecycle.md)):

1. the per-unit `NS.Units.IsEnabled(unit)` flag (target/focus ship `false`) — a different setting from the addon-wide `enabled` row, and §15 forbids conflating them: that one is *whether the addon runs at all* and reaches this ladder only through the latch, the three are *which bars exist*. Neither is the pre-v4 `hidden` global returning; that key was dropped because nothing in the UI could clear it. Past this step, being **unlocked** answers *shown* and skips steps 2 and 3, so a bar can be seen and placed with no target and under any `visibility` (this rung was test mode's until options-ui-§15 exempted the addon from that row — the lock is the one preview switch now),
2. the `visibility` dropdown — `always` / `inCombat` / `outOfCombat` / `never`, where an unrecognized stored value reads as `always` (a hidden addon with no visible cause is the worse failure). The combat arms key off actual player combat, **not** `InCombatLockdown()`, which lags the `PLAYER_REGEN_DISABLED` transition — see `docs/midnight-quirks.md`,
...
| `PLAYER_REGEN_DISABLED` (enter combat) | `addon:OnEnterCombat` | **Re-locks** the bars if they are unlocked (`locked` true through `NS.SetByPath`, one chat line, a panel refresh), so the fight starts on live data. Either that OR `VISIBILITY` + `REPAINT`, never both: the `locked` onChange already publishes `APPEARANCE`, whose pass calls `NS.ApplyVisibility` itself, so publishing again would run the ladder over all three bars twice. |
| `PLAYER_REGEN_ENABLED` (leave combat) | `addon:OnLeaveCombat` | Publishes `VISIBILITY` + `REPAINT` — nothing else. Per options-ui-§2 the settings panel **refuses** to open in combat (the `InCombatLockdown()` check and the gray `STRINGS.COMBAT_REFUSED` notice are `libs/LibKa0s/Options.lua`'s, printed through the descriptor's `print` so the line still carries `[AT]`; reached via `NS.OpenOptionsPanel` in `settings/OptionsSetup.lua`) rather than deferring, so there is no combat-deferred `/at config` for `OnLeaveCombat` to replay. It is the sole handler of `PLAYER_REGEN_ENABLED`, and its only job is re-evaluating the `visibility` gate and repainting. |
...
The TOC declares two: `AbsorbTrackerDB, AbsorbTrackerPerfDB`. Only the first is the addon's own state — the perf ring is a separate global written straight by `LibKa0s-Perf-1.0`, deliberately outside any profile. Its owner and writer are named in [schema.md → Named non-setting state](./schema.md#named-non-setting-state). With AceDB `AbsorbTrackerDB` holds the full profile structure (`profiles`, `profileKeys`, `char` map, etc.); `db.profile` holds the six flat globals (`enabled`, `visibility`, `scale`, `alpha`, `locked`, `throttleWindow`) plus `units.{player,target,focus}` (each unit's own appearance + position table), and the persisted schema-version stamp lives at `db.global.schemaVersion` (account-wide, the DB-wide marker — currently `5`; v3 introduced `profile.units`, v4 dropped the dead `hidden` master toggle, v5 mapped `showOnlyInCombat` onto `visibility`) with a second, **per-profile** stamp at `db.profile.schemaVersion` gating the per-profile v3 lift (a documented savedvariables-§1 deviation — see [profiles.md](./profiles.md)). Without AceDB, `NS:InitDB` builds a minimal `{ profile = AbsorbTrackerDB, global = {} }` shim so `GetSetting` reads and `NS.SetByPath` writes stay consistent across the two modes. Either way, `NS:RunMigrations` runs once at init and backfills any missing profile key (flat or per-unit) from `NS.defaults.profile`.
