...
| `PLAYER_REGEN_DISABLED` (enter combat) | `addon:OnEnterCombat` | Ends test mode first if it is on (`state.testMode` off through `NS.SetByPath`, one chat line, a panel refresh). Then publishes `VISIBILITY` + `REPAINT` — re-evaluates the `visibility` gate so any eligible bar appears (and repaints fresh) the moment combat starts. |
