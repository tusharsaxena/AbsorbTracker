...
- **Canvas-layout parent + sub-pages, unified breadcrumb header.** `OptionsPanel.lua` is the registration shell; the toolkit it dispatches to (`Panel/Helpers.lua` for layout + `Panel/ScrollPatch.lua` for the scrollbar override + `Panel/Widgets.lua` for `RenderField` / `RenderSchema` + `Panel/About.lua` for the top-level page) decorates a single shared `AddonTable.Helpers` table. The shell registers a custom canvas frame for the "Ka0s Absorb Tracker" parent (which renders an about page via `Helpers.BuildMainContent`: logo + TOC `Notes` + slash-command list) and one canvas frame per sub-page. Every page wears the same header (`GameFontNormalHuge` title in `"Ka0s Absorb Tracker ▸ <Page>"` breadcrumb form, where the separator is the inline-atlas escape `|A:common-icon-forwardarrow:16:16|a` rather than a font glyph, + `Options_HorizontalDivider` atlas + optional Defaults button). The five sub-pages (General, Bar, Border, Font, Profiles) hold every setting; the parent never holds settings of its own. Profile / `/at set` refresh runs `Helpers.RefreshAllPanels`, which walks per-widget refresher closures rather than per-page namespaces.
