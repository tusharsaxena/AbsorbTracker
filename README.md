...
`/at lock` to pin it there. While the bars are unlocked each one paints a partial fill and grows a
small handle strip above it with the unit's name on it, so there is something to grab even with no
shield up (drag the strip or the bar itself) — and an enabled Target or Focus bar
...
| How do I move a bar? | Type `/at unlock`, drag the bar you want where you want it, then `/at lock`. Each bar remembers its own position. Use `/at resetposition` to snap all of them back to their default spots. Unlocking also brings up the Target and Focus bars with nothing targeted, so you can place those too. |
...
| How do I see debug logs? | `/at debug` toggles the on-screen debug window; `/at debug on` / `off` starts and stops logging there instead of in chat. You can also toggle the window with the **Debug console** checkbox on the General page. Logging resets to off every reload. To send logs with a bug report, follow [Reporting a bug](#reporting-a-bug). |
