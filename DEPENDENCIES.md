...
`tests/test_vendor_sync.lua` shells out to `git` to prove the vendored `libs/LibKa0s/` and
`tests/_kit/` payloads are byte-identical to the LibKa0s release the README claims to bundle:

```
tests/test_vendor_sync.lua:53    io.popen(('git -C "%s" %s 2>/dev/null'):format(SIBLING, args), "r")
```

It runs `git show` and `git ls-tree` against a **sibling checkout at `../LibKa0s`**
(`tests/test_vendor_sync.lua:38`).
...
**quiet-skip** — deliberately, and the skip is named in the case rather than hidden
(`tests/test_vendor_sync.lua:105-116`, `:140` and `:146`, the `if not tag then return end` guard).
...
tests/test_vendor_sync.lua:89    io.popen(('ls -A "%s" 2>/dev/null'):format(dir))
```

`tests/test_vendor_sync.lua:90` falls back to `dir /b` for `cmd.exe`, so that suite survives a Windows
...
- **LuaFileSystem** — *not* used. `tests/test_vendor_sync.lua:74-76` says so explicitly and shells out
